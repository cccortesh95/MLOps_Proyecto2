...
| `NoSuchBucket` al registrar modelo en MLflow | El job `minio-create-bucket` corrió antes de que MinIO estuviera listo | Recrear el job: `kubectl delete job minio-create-bucket -n mlops && kubectl apply -f k8s/minio/job-create-bucket.yaml`. Verificar con `kubectl logs job/minio-create-bucket -n mlops` |
